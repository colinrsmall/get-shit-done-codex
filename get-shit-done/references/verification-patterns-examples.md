...
Extended examples and artifact-specific checks. Core rules live in `get-shit-done/references/verification-patterns.md`.
...
**Existence check:**
- Confirm `.env.example` or `.env.template` lists the required variable
- Confirm the app's config loader/schema includes the variable

**Substantive check:**
- Prefer runtime validation (app start/config check) instead of reading `.env` contents
...
**Wiring check:**
- Use `grep` tool to confirm the variable is referenced in code (e.g., `process.env.VAR_NAME`)
- Use `grep` tool to confirm the variable is in the env validation schema (if present)
...
Key principles:
- Assistant sets up verification environment BEFORE presenting checkpoints
