...
Model training data is 6-18 months stale. Always verify.
...
2. **Context7 for each option:**
   ...
   ```
    ...
    - mcp__context7__query-docs (use specific queries; ask for API snippets when needed)
   ```

3. **Official docs** for anything Context7 lacks.

4. **WebFetch** for comparisons (use a search URL):
...
Run the discovery:
- Use WebFetch for current info (search URL or curated sources)
